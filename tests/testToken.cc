...
void assertString(const std::string &result,
...
int main(void) {
  std::unordered_map<std::string, std::string> correct_test_suite{
      ...
      {"1^2", "INT( 1 ) EXP INT( 2 )"},
      {"(1+2)()+3",
       "LPARA INT ( 1 ) PLUS INT ( 2 ) WS LPARA RPARA PLUS INT ( 3 )"},
      {"1+3/4x5-7^8/9x3.14159\n",
       "INT( 1 ) PLUS INT( 3 ) DIV INT( 4 ) MUL INT( 5 ) MINUS INT( 7 ) EXP "
       "INT( 8 ) DIV INT( 9 ) MUL FLOAT( 3.14159 ) END"},
      {"1+3 x   4 -   5 \n",
       "INT( 1 ) PLUS INT( 3 ) MUL WS INT( 4 ) WS MINUS WS INT( 5 ) WS END"},
      {"(( 3.2 + 4.97)x27 / -9 )^(1/5)\n",
       "LPARA LPARA WS FLOAT( 3.2 ) WS PLUS WS FLOAT( 4.97 ) RPARA MUL INT( 27 "
       ") WS DIV WS MINUS INT( 9 ) WS RPARA EXP LPARA INT( 1 ) DIV INT( 5 ) "
       "RPARA END"},
  };
  std::unordered_map<std::string, std::string> error_test_suite{
      {"1+3*2", "Invalid token at index 3: *"},
      {"1.2.3", "Invalid token at index 3: ."},
  ...
  TokenGen &sut = injector.get<TokenGen &>();
  int i = 1;
